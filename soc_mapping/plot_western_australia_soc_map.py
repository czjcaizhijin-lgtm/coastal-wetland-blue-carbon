...
# ==========================================
# 2. 读取与推演数据 (澳大利亚西海岸空间过滤)
# ==========================================
print("正在加载模型并提取澳大利亚西海岸光滩数据...")
model = joblib.load(model_path)
file_list = glob.glob(os.path.join(folder_path, "*.csv"))
df_list = []

# 锁定澳大利亚西海岸核心区域的提取范围
lon_min, lon_max = 112.0, 118.0
lat_min, lat_max = -36.0, -14.0
...
if not df_list:
    ...
    if len(valid_soc) > 0:
        ...
        # 4. 局部制图
        # ==========================================
        print("正在渲染统一色标体系下的澳大利亚西海岸高清地图...")
        fig = plt.figure(figsize=(14, 12), dpi=300)

        ax = plt.axes(projection=ccrs.PlateCarree())
        # 精确锁定展示的视觉范围
        ax.set_extent([113.0, 117.0, -34.0, -16.0], crs=ccrs.PlateCarree())
        ...
        cbar.set_label('Predicted Unvegetated Intertidal Flats Deep SOC - West Coast of Australia', fontsize=16, fontweight='bold')

      #  plt.title('West Coast of Australia Tidal Flat SOC Patterns', fontsize=24, fontweight='bold', pad=20)

        save_path = os.path.join(save_directory, 'Australia_West_Coast_SOC_Zoom_Unified.png')
        plt.savefig(save_path, dpi=600, bbox_inches='tight', pad_inches=0.2)

        print(f"统一色标的澳大利亚西海岸大图生成完毕！查看路径: {save_path}")
        plt.show()
    else:
        print("栅格化后该区域无有效数据。")
