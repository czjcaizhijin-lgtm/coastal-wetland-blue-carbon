...
# ==========================================
# 2. 读取与推演数据 (欧洲北海与瓦登海空间过滤)
# ==========================================
print("🚀 正在加载模型并提取欧洲北海与瓦登海光滩数据...")
model = joblib.load(model_path)
file_list = glob.glob(os.path.join(folder_path, "*.csv"))
df_list = []

# 🌟 锁定欧洲西北部沿海（北海及瓦登海核心带）的提取范围
lon_min, lon_max = -4.0, 12.0
lat_min, lat_max = 50.0, 58.0
...
if not df_list:
    ...
    # 3. 栅格化处理
    # ==========================================
    grid_res = 0.05  # 保持高分辨率以捕捉狭长的近岸潮间带泥滩
    ...
    if len(valid_soc) > 0:
        ...
        # 4. 局部制图
        # ==========================================
        print("🗺️ 正在渲染统一色标体系下的瓦登海高清局部地图...")
        fig = plt.figure(figsize=(14, 11), dpi=300)

        ax = plt.axes(projection=ccrs.PlateCarree())
        # 精确锁定英国东岸、荷兰、德国到丹麦西岸的视觉展示框
        ax.set_extent([-2.0, 10.0, 51.0, 56.5], crs=ccrs.PlateCarree())
        ...
        cbar.set_label('Predicted Unvegetated Intertidal Flats Deep SOC - Wadden Sea & North Sea', fontsize=16, fontweight='bold')

      #  plt.title('Wadden Sea and North Sea Tidal Flat SOC Patterns', fontsize=24, fontweight='bold', pad=20)

        save_path = os.path.join(save_directory, 'Wadden_Sea_SOC_Zoom_Unified.png')
        plt.savefig(save_path, dpi=600, bbox_inches='tight', pad_inches=0.2)

        print(f"🎉 统一色标的北海/瓦登海大图生成完毕！查看路径: {save_path}")
        plt.show()
    else:
