...
# 2. 读取与推演数据 (东南亚群岛)
# ==========================================
print("🚀 正在加载模型并提取东南亚群岛光滩数据...")
model = joblib.load(model_path)
file_list = glob.glob(os.path.join(folder_path, "*.csv"))
df_list = []

lon_min, lon_max = 95.0, 145.0
lat_min, lat_max = -12.0, 10.0
...
if not df_list:
    ...
    ax.add_feature(cfeature.COASTLINE, linewidth=0.8, color='#333333', zorder=4)
    ...
    # 🌟 修改色标渲染逻辑，强制显示这 9 个固定刻度
    cbar = plt.colorbar(mesh, orientation='horizontal', pad=0.1, aspect=50, shrink=0.7, ticks=fixed_bounds)
    cbar.ax.set_xticklabels([f"{b:.1f}" for b in fixed_bounds], fontsize=12)
    cbar.set_label('Predicted Unvegetated Intertidal Flats Deep SOC - Unified Decile Classification', fontsize=16, fontweight='bold')

    #plt.title('Southeast Asian Archipelago Tidal Flat SOC Patterns', fontsize=24, fontweight='bold', pad=20)

    save_path = os.path.join(save_directory, 'Southeast_Asia_SOC_Zoom_Unified.png')
    plt.savefig(save_path, dpi=600, bbox_inches='tight', pad_inches=0.2)

    print(f"🎉 统一色标的大图生成完毕！查看路径: {save_path}")
    plt.show()