...
# 2. 读取与推演数据 (亚马逊河口空间过滤)
# ==========================================
print("🚀 正在加载模型并提取亚马逊河口光滩数据...")
model = joblib.load(model_path)
file_list = glob.glob(os.path.join(folder_path, "*.csv"))
df_list = []

# 锁定亚马逊核心区域的展示范围
lon_min, lon_max = -55.0, -45.0
lat_min, lat_max = -4.0, 4.0
...
if not df_list:
    ...
    if len(valid_soc) > 0:
        ...
        cbar.set_label('Predicted Bare Tidal Flat Deep SOC - Unified Decile Classification', fontsize=16,
                       fontweight='bold')

        plt.title('Amazon Delta Tidal Flat SOC Patterns', fontsize=24, fontweight='bold', pad=20)

        save_path = os.path.join(save_directory, 'Amazon_Delta_SOC_Zoom_Unified.png')
