...
save_directory = r'D:\PyCharm2020(64bit)\pythonProject\反演代码'
...
# ==========================================
# 2. 读取与推演数据 (中国黄海沿岸空间过滤)
# ==========================================
print("正在加载模型并提取中国黄海沿岸光滩数据...")
model = joblib.load(model_path)
file_list = glob.glob(os.path.join(folder_path, "*.csv"))
df_list = []

# 锁定中国黄海沿岸核心区域的提取范围
lon_min, lon_max = 118.5, 127.0
lat_min, lat_max = 31.0, 41.0
...
if not df_list:
    ...
    if len(valid_soc) > 0:
        ...
        # 4. 局部制图
        # ==========================================
        print("正在渲染统一色标体系下的中国黄海沿岸高清地图...")
        fig = plt.figure(figsize=(14, 12), dpi=300)

        ax = plt.axes(projection=ccrs.PlateCarree())
        # 精确锁定展示的视觉范围
        ax.set_extent([119.0, 126.0, 32.0, 40.0], crs=ccrs.PlateCarree())
        ...
        cbar.set_label('Predicted Unvegetated Intertidal Flats Deep SOC - Yellow Sea Coast of China', fontsize=16, fontweight='bold')

       # plt.title('Yellow Sea Coast of China Tidal Flat SOC Patterns', fontsize=24, fontweight='bold', pad=20)

        save_path = os.path.join(save_directory, 'China_Yellow_Sea_Coast_SOC_Zoom_Unified.png')
        plt.savefig(save_path, dpi=600, bbox_inches='tight', pad_inches=0.2)

        print(f"统一色标的中国黄海沿岸大图生成完毕！查看路径: {save_path}")
        plt.show()
    else:
        print("栅格化后该区域无有效数据。")
